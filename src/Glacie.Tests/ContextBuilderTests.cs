...
namespace Glacie.Tests
{
    public sealed class ContextBuilderTests : GlacieTests
    {
        ...
        [Fact]
        public void ThrowsIfNoTarget()
        {
            AssertGxError("ContextTargetIsNotSpecified", () =>
            {
                using var context = Context.Create(c =>
                {
                });
        ...
        [Fact]
        public void ThrowsIfMultipleTargets()
        {
            AssertGxError("ContextMultipleTargets", () =>
            {
                using var targetDatabase1 = ArzDatabase.Create();
                using var targetDatabase2 = ArzDatabase.Create();
                using var context = Context.Create(c =>
                {
                    c.Target(t => t.Database(targetDatabase1));
                    c.Target(t => t.Database(targetDatabase2));
                });
            });
        }
