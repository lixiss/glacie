...
namespace Glacie.Metadata
{
    public sealed class TemplateMetadataProvider : MetadataProvider
    {
        ...
        private readonly IVirtualPathMapper? _templateNameMapper;
        ...
        public TemplateMetadataProvider(TemplateProvider templateProvider,
            IVirtualPathMapper? templateNameMapper,
        ...
        private RecordTypeBuilder GetOrCreateRecordType(in VirtualPath templateName)
        {
        ...
        private bool TryGetOrCreateRecordType(in VirtualPath templateName,
            [NotNullWhen(returnValue: true)] out RecordTypeBuilder? result)
        {
            if (_databaseDefinitionBuilder.TryGetRecordDefinition(templateName, out result))
            {
                return true;
            }

            // Map and normalize template name.
            VirtualPath vTemplateName;
            ...
            vTemplateName = new VirtualPath(vTemplateName)
                .Normalize(VirtualPathNormalization.Standard);

            // TODO: lookup for possibly re-mapped value before resolving template
            if (_databaseDefinitionBuilder.TryGetRecordDefinition(vTemplateName, out result))
            {
                return true;
            }
        ...
        private void ParseTemplate(ref Context context, Template template)
        {
            ...
            if (context.HasIncludeVariables)
            {
                ...
                {
                    var includedRecordDefinition = GetOrCreateRecordType(includeVariable.DefaultValue);
