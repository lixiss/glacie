...
namespace Glacie.Data.Templates
{
    ...
    public sealed class TemplateReader
    {
        ...
        public Template Read(in Path1 path)
            => Read(path, _defaultEncoding);

        public Template Read(in Path1 path, Encoding encoding)
        {
            using var stream = File.OpenRead(path.ToString());
            return Read(stream, encoding, path, leaveOpen: false);
        }

        public Template Read(Stream stream, in Path1 path = default, bool leaveOpen = false)
            => Read(stream, _defaultEncoding, in path, leaveOpen);

        public Template Read(Stream stream, Encoding encoding, in Path1 path = default, bool leaveOpen = false)
        {
            ...
            return Read(textReader, in path);
        }

        public Template Read(TextReader reader, in Path1 path = default, bool leaveOpen = false)
        {
            Check.Argument.NotNull(reader, nameof(reader));
            try
            {
                return ReadInternal(reader, in path);
            }
            finally
            {
        ...
        private Template ReadInternal(TextReader reader, in Path1 path)
        {
            ...
            try
            {
                return ParseTemplate(ref tokenReader, in path);
            }
            finally
            {
                tokenReader.Dispose();
        ...
        private Template ParseTemplate(ref TokenReader reader, in Path1 path)
        {
        ...
        private TemplateGroup ParseGroup(ref TokenReader tokenReader, in Path1 path)
        {
        ...
        private TemplateVariable ParseVariable(ref TokenReader tokenReader, in Path1 path)
        {
            var beginToken = tokenReader.Read();
            if (beginToken.Type != TokenType.BeginBlock)
            {
        ...
        private List<string> ParseFileNameHistoryEntry(ref TokenReader tokenReader, in Path1 path)
        {
            var beginToken = tokenReader.Read();
            if (beginToken.Type != TokenType.BeginBlock)
            {
        ...
        private string ReadAssignmentAndValue(ref TokenReader tokenReader, in Path1 path)
        {
            var token = tokenReader.Read();
        ...
        private void ReadStringProperty(ref string? target, in Token currentToken, ref TokenReader tokenReader, in Path1 path)
        {
            if (target != null)
            {
                throw DiagnosticFactory
